...
router.post('/scrape-profile', async (req: Request, res: Response) => {
  try {
    ...
    if (account.connectionMode !== 'browser' || !account.passwordEncrypted)
      return res.status(400).json({ error: 'Auto-fetch requires a browser-mode account with a password.' });
  ...
  try {
    ...
    const created = await prisma.lead.createMany({
      data: newLeads.map((l) => ({
        ...
        headline: l.headline ?? null, company: l.company ?? null, location: l.location ?? null, status: 'new',
      })),
    });
    res.status(201).json({
      created: created.count, skipped: leads.length - newLeads.length,
